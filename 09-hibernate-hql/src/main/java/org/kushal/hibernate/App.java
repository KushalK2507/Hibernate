...
public class App {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      Query query1 =
          session.createQuery(
              "select userdId from users"); // We can also use the slect in HQL to get
      ...
      Query query3 =
          session.createQuery("from users where userId > :userId and userName = :userName1");
      ...
      // Use of where clause
      List<Users> listUser =
          session.createQuery("from users where username='Hello'").getResultList();

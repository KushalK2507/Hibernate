...
public class App {
  ...
  public static void main(String[] args) {
    ...
    user.setUserName(
        "Kushal"); // At this time user object is transient object and not persistent as it is yet
    // not persisted in DB.

    session.getTransaction();
    session.save(
        user); // At this time user object is persistent and hibernate tracks this object and
    // saves it.

    user.setUserName(
        "Hello"); // At this time hibernate will run the uppdate statement update the user
    ...
    user.setUserName(
        "After Comit"); // This updation will not reflect in the DB as session is closed.
